...
// --- Toolchain sidecar tests ---

func TestBuildPod_WithToolchainSidecar(t *testing.T) {
	client := fake.NewSimpleClientset()
	mgr := New(client, slog.Default())
	...
	spec := AgentPodSpec{
		...
		ToolchainSidecar: &ToolchainSidecarSpec{
			...
			Image:   "ghcr.io/groblegark/toolchain:latest",
		},
	}

	pod := mgr.buildPod(spec)

	// Toolchain sidecar should appear as an init container (native sidecar pattern).
	var tc *corev1.Container
	for i := range pod.Spec.InitContainers {
		if pod.Spec.InitContainers[i].Name == ToolchainContainerName {
			tc = &pod.Spec.InitContainers[i]
			break
		}
	}
	if tc == nil {
		t.Fatal("expected toolchain init container")
	}

	// Image.
	if tc.Image != "ghcr.io/groblegark/toolchain:latest" {
		t.Errorf("toolchain image = %q, want %q", tc.Image, "ghcr.io/groblegark/toolchain:latest")
	}

	// RestartPolicy: Always (native sidecar).
	if tc.RestartPolicy == nil || *tc.RestartPolicy != corev1.ContainerRestartPolicyAlways {
		t.Error("toolchain sidecar restartPolicy should be Always")
	}

	// Volume mounts: workspace + tmp.
	mountMap := make(map[string]string)
	for _, m := range tc.VolumeMounts {
		mountMap[m.Name] = m.MountPath
	}
	if mountMap[VolumeWorkspace] != MountWorkspace {
		t.Errorf("toolchain workspace mount = %q, want %q", mountMap[VolumeWorkspace], MountWorkspace)
	}
	if mountMap[VolumeTmp] != MountTmp {
		t.Errorf("toolchain tmp mount = %q, want %q", mountMap[VolumeTmp], MountTmp)
	}

	// Security context.
	if tc.SecurityContext == nil {
		t.Fatal("toolchain security context is nil")
	}
	if *tc.SecurityContext.RunAsUser != AgentUID {
		t.Errorf("RunAsUser = %d, want %d", *tc.SecurityContext.RunAsUser, AgentUID)
	}
	if *tc.SecurityContext.RunAsGroup != AgentGID {
		t.Errorf("RunAsGroup = %d, want %d", *tc.SecurityContext.RunAsGroup, AgentGID)
	}
	if *tc.SecurityContext.AllowPrivilegeEscalation {
		t.Error("AllowPrivilegeEscalation should be false")
	}
	if tc.SecurityContext.Capabilities == nil || len(tc.SecurityContext.Capabilities.Drop) == 0 {
		t.Error("should drop ALL capabilities")
	}

	// Env vars on the sidecar itself.
	envMap := make(map[string]string)
	for _, e := range tc.Env {
		envMap[e.Name] = e.Value
	}
	if envMap["GT_TOOLCHAIN_CONTAINER"] != ToolchainContainerName {
		t.Errorf("GT_TOOLCHAIN_CONTAINER = %q, want %q", envMap["GT_TOOLCHAIN_CONTAINER"], ToolchainContainerName)
	}
	if envMap["GT_TOOLCHAIN_IMAGE"] != "ghcr.io/groblegark/toolchain:latest" {
		t.Errorf("GT_TOOLCHAIN_IMAGE = %q, want %q", envMap["GT_TOOLCHAIN_IMAGE"], "ghcr.io/groblegark/toolchain:latest")
	}
	if envMap["GT_TOOLCHAIN_PROFILE"] != "toolchain-full" {
		t.Errorf("GT_TOOLCHAIN_PROFILE = %q, want %q", envMap["GT_TOOLCHAIN_PROFILE"], "toolchain-full")
	}

	// ImagePullPolicy.
	if tc.ImagePullPolicy != corev1.PullAlways {
		t.Errorf("ImagePullPolicy = %q, want Always", tc.ImagePullPolicy)
	}
}

func TestBuildPod_WithoutToolchainSidecar(t *testing.T) {
	client := fake.NewSimpleClientset()
	mgr := New(client, slog.Default())
	...
	for _, ic := range pod.Spec.InitContainers {
		if ic.Name == ToolchainContainerName {
			t.Error("toolchain init container should not exist when ToolchainSidecar is nil")
		}
	}
}

func TestBuildPod_ToolchainEnvVarsInAgent(t *testing.T) {
	client := fake.NewSimpleClientset()
	mgr := New(client, slog.Default())
	ctx := context.Background()
	...
	pod, _ := client.CoreV1().Pods("gastown").Get(ctx, "gt-gastown-crew-tc-env", metav1.GetOptions{})
	envMap := make(map[string]string)
	for _, e := range pod.Spec.Containers[0].Env {
		envMap[e.Name] = e.Value
	}
	...
	if envMap["GT_TOOLCHAIN_CONTAINER"] != ToolchainContainerName {
		t.Errorf("agent GT_TOOLCHAIN_CONTAINER = %q, want %q", envMap["GT_TOOLCHAIN_CONTAINER"], ToolchainContainerName)
	}
	if envMap["GT_TOOLCHAIN_IMAGE"] != "ghcr.io/groblegark/toolchain:latest" {
		t.Errorf("agent GT_TOOLCHAIN_IMAGE = %q, want %q", envMap["GT_TOOLCHAIN_IMAGE"], "ghcr.io/groblegark/toolchain:latest")
	}
	if envMap["GT_TOOLCHAIN_PROFILE"] != "toolchain-full" {
		t.Errorf("agent GT_TOOLCHAIN_PROFILE = %q, want %q", envMap["GT_TOOLCHAIN_PROFILE"], "toolchain-full")
	}
}

func TestBuildPod_ToolchainEnvVarsNotInAgentWithoutSidecar(t *testing.T) {
	client := fake.NewSimpleClientset()
	mgr := New(client, slog.Default())
	ctx := context.Background()
	...
	pod, _ := client.CoreV1().Pods("gastown").Get(ctx, "gt-gastown-crew-no-tc-env", metav1.GetOptions{})
	for _, e := range pod.Spec.Containers[0].Env {
		if e.Name == "GT_TOOLCHAIN_CONTAINER" || e.Name == "GT_TOOLCHAIN_IMAGE" || e.Name == "GT_TOOLCHAIN_PROFILE" {
			t.Errorf("unexpected toolchain env var %q on agent without sidecar", e.Name)
		}
	}
}

func TestBuildToolchainResources_Default(t *testing.T) {
	client := fake.NewSimpleClientset()
	mgr := New(client, slog.Default())
	...
	cpuReq := tc.Resources.Requests[corev1.ResourceCPU]
	if cpuReq.String() != ToolchainDefaultCPURequest {
		t.Errorf("CPU request = %s, want %s", cpuReq.String(), ToolchainDefaultCPURequest)
	}
	memReq := tc.Resources.Requests[corev1.ResourceMemory]
	if memReq.String() != ToolchainDefaultMemRequest {
		t.Errorf("Memory request = %s, want %s", memReq.String(), ToolchainDefaultMemRequest)
	}
	cpuLimit := tc.Resources.Limits[corev1.ResourceCPU]
	if cpuLimit.String() != ToolchainDefaultCPULimit {
		t.Errorf("CPU limit = %s, want %s", cpuLimit.String(), ToolchainDefaultCPULimit)
	}
	memLimit := tc.Resources.Limits[corev1.ResourceMemory]
	if memLimit.String() != ToolchainDefaultMemLimit {
		t.Errorf("Memory limit = %s, want %s", memLimit.String(), ToolchainDefaultMemLimit)
	}
}

func TestBuildToolchainResources_Custom(t *testing.T) {
	client := fake.NewSimpleClientset()
	mgr := New(client, slog.Default())
