...
func (b *Bot) showResolvedDecisionView(callback slack.InteractionCallback, decision *rpcclient.Decision) {
	...
	if decision.Context != "" {
		// Format context nicely (shows full context inline)
...
func (b *Bot) NotifyNewDecision(decision rpcclient.Decision) error {
	...
	// Show full context inline (no truncation, no "Show Full Context" button needed)
	if decision.Context != "" {
		contextText := formatContextForSlack(decision.Context)
		blocks = append(blocks,
			slack.NewSectionBlock(
...
// Removes internal fields like _type and session_id from display.
// Shows full context inline (up to Slack's ~3000 char block limit).
func formatContextForSlack(context string) string {
	...
	if obj, ok := parsed.(map[string]interface{}); ok {
		...
		delete(obj, "_type")
		delete(obj, "_value")
		delete(obj, "session_id") // Don't expose session_id to users
