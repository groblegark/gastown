// Package ui provides terminal styling utilities.
package ui
...
func TruncateLines(text string, maxLines, contextLines int) string {
	...
	result.WriteString("\n")
	result.WriteString(MutedStyle.Render(strings.Repeat("─", 40)))
	result.WriteString("\n")
	result.WriteString(MutedStyle.Render("... [" + itoa(hiddenLines) + " lines hidden - use --full to see complete text] ..."))
	result.WriteString("\n")
	result.WriteString(MutedStyle.Render(strings.Repeat("─", 40)))
	result.WriteString("\n")
...
func TruncateChars(text string, maxChars, contextChars int) string {
	...
	return beginText + "\n" + MutedStyle.Render("... ["+itoa(hiddenChars)+" chars hidden] ...") + "\n" + endText
}
