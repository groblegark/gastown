...
func TestGetAgentBeadID_UsesRigPrefix(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			// Polecats use rig prefix (reverted from hq- town-level beads)
...
// Note: matchesAdvice and filterApplicableAdvice were moved to beads CLI.
...
// Note: TestAdviceMatchesRig was removed because adviceMatchesRig() was deleted.
// Rig-scoping is now handled by beads CLI in `bd advice list --for=<agent-id>`.
// See gt-ltd22t for details on the refactoring.
