// Liftoff test: 2026-02-03T15:09:50
...
// Polecat agent beads are stored in rig beads with the rig's configured prefix.
...
// setupClaudeSymlink creates symlinks from the worktree to the shared polecats/ configuration:
...
// traversal), so symlinks are required for polecats to inherit shared configuration.
//
...
// Fix for: gt-9212oz (polecats not loading MCP servers)
func (m *Manager) setupClaudeSymlink(clonePath string) error {
	// Set up .claude symlink
	if err := m.setupSymlink(clonePath, ".claude"); err != nil {
		return err
	}

	// Set up .mcp.json symlink for MCP server configuration
	if err := m.setupSymlink(clonePath, ".mcp.json"); err != nil {
...
// setupSymlink creates a symlink from the worktree to the shared polecats/ directory.
// From: polecats/<name>/<rigname>/<target> -> polecats/<target>
// Relative path: ../../<target>
func (m *Manager) setupSymlink(clonePath, target string) error {
	symlinkPath := filepath.Join(clonePath, target)
	...
	// Check if the target exists in shared polecats/ directory
	targetPath := filepath.Join(m.rig.Path, "polecats", target)
