...
func (b *Beads) run(args ...string) ([]byte, error) {
	...
	if err != nil {
		...
		// Check stdout regardless of stderr content — bd may produce
		// informational warnings on stderr (e.g. staleness, no git repo)
		// alongside a JSON error on stdout. (bd-hm48s.4)
		if stdout.Len() > 0 {
...
func filterBeadsEnv(environ []string) []string {
	...
	for _, env := range environ {
		...
		// BD_ACTOR, BD_DAEMON_HOST - direct beads config and daemon routing
		// BEADS_* - beads configuration
		...
		if strings.HasPrefix(env, "BD_ACTOR=") ||
			strings.HasPrefix(env, "BD_DAEMON_HOST=") ||
