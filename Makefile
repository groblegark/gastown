.PHONY: build install clean test test-safe generate deploy proto
...
# Read platform version from env file if it exists
PLATFORM_VERSION := $(shell grep '^PLATFORM_VERSION=' platform-versions.env 2>/dev/null | cut -d= -f2)
